...
import { clientes as clientesDados } from "@/data/clientes";

export default function Page() {
    const [query, setQuery] = useState("");
    const [dados, setDados] = useState(clientesDados);
    ...
    return (
        <div className="flex-1 min-h-screen flex flex-col">
            <PageHeader titulo="Clientes" descricao="Gerenciamento de clientes" botaoNome="+ novo cliente" onCreate={() => { setEditing(null); setModalOpen(true); }} />
            <div className="p-8 bg-white mx-8 my-8 rounded-lg border border-gray-200 flex flex-col flex-1">
                <SearchBar value={query} onChange={setQuery} />
                <TabelaDados dados={filtered} tipo="cliente" onEdit={(item) => { setEditing(item); setModalOpen(true); }} />
            </div>
